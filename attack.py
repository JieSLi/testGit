...
class Attack: 
    def __init__(self, sess, loss_fn, phrase_length, max_audio_len,
        ...
        print("\nOriginal Shape: ",tf.shape(self.original))
        print("Original Type: ",type(self.original))
        print("Original:",self.original)


        sess.run(tf.variables_initializer([self.delta]))
        sess.run(self.original.assign(np.array(audio)))
        sess.run(self.lengths.assign((np.array(a_lengths)-1)//320))
        sess.run(self.mask.assign(np.array([[1 if i < l else 0 for i in range(self.max_audio_len)] for l in a_lengths])))
        sess.run(self.cwmask.assign(np.array([[1 if i < l else 0 for i in range(self.phrase_length)] for l in (np.array(a_lengths)-1)//320])))
        ...
        print("TYPE :",type(self.original.eval()))
        ...
        if loss_fn == "CTC":
            ...
            if np.isinf(l2penalty):
                print("Changing Loss")
                smooth_loss = 0
                #delta_2 is our delta for computation purposes
                delta_2 = self.new_input -  self.original
                #for some reason computing size here gets us the length of the input
                size = tf.size(delta_2)
                #but getting the first element here will allow us to slice properly
                delta_2 = delta_2[0]
            
                #create array of shifted deltas
                delta_shifts = []
                for i in range(-5,6):
                    #cut off first and last features that are messed up b/c of how roll works in tensorflow (there is  wrap aound)
                    temp_tens = tf.roll(delta_2,i,0)[10:size - 10]
                    delta_shifts.append(temp_tens)

                # for each shifted delta, compute squared difference --> (f(j) -  j  - (f(k) - k)) in smooth loss function
                # each shift representes a neighbor, so we will compute this for each "neighbor" of j
            
                #resize delta_2 to align sizes
                delta_2 = delta_2[10:size - 10]
                for del_shift in delta_shifts:
                    #square or abs?
                    smooth_loss += tf.math.reduce_mean(tf.math.abs(delta_2 - del_shift))
                #loss = tf.reduce_mean((self.new_input-self.original)**2,axis=1) + l2penalty*ctcloss
                #hyper parameters
                a = 0.05
                c = 0.9
                loss = a * smooth_loss + c * ctcloss
            else:
                loss = ctcloss
            self.expanded_loss = tf.constant(0)
        print("Loss in constructor: ",loss)
        self.loss = loss
        self.ctcloss = ctcloss
        self.smoothloss = smooth_loss
    ...
    def attack(self, audio, lengths, target, finetune=None):
        ...
        #writer = tf.summary.filewriter('./attack_graph', sess.graph)
        
        print("\n\n-------------------------------")
        print("Variables after initial assignments")
        print("Original: ",np.shape(self.original.eval()))
        print("Length: ",self.lengths.eval())
        print("Mask: ",np.shape(self.mask.eval()))
        print("CWMask: ",np.shape(self.mask.eval()))
        print("Target phrase: ",self.target_phrase.eval())
        print("Resacle: ",self.rescale.eval())
        print("\n\n----------------------------------------")
        ...
        for i in range(MAX):
            ...
            d, el, cl, sl, l, logits, new_input, _ = sess.run((self.delta, self.expanded_loss,
                                                           self.ctcloss, self.smoothloss, self.loss,
                                                           self.logits, self.new_input,
                                                           self.train),
                                                          feed_dict)
            #print("New Input Shape: ",tf.shape(new_input))
...
def main():
    ...
    with tf.Session() as sess:
        ...
        attack = Attack(sess, 'CTC', len(phrase), maxlen,
                        ...
                        audio = audios, a_lengths = lengths, target = [[toks.index(x) for x in phrase]]*len(audios),
                        mp3=args.mp3,
